...
static int timer_comp(void *ti, void *tj) {
    tiny_timer_node *timeri = (tiny_timer_node *)ti;
    tiny_timer_node *timerj = (tiny_timer_node *)tj;
...
int timer_init() {
    int rc;
    rc = pq_init(&timer, QUEUE_SIZE,timer_comp);
...
int find_timer() {
    tiny_timer_node *timer_node;
    int time = TINY_TIMER_INFINITE;
    int rc;

    while (!pq_is_empty(&timer)){
        debug("find_timer");
        time_update();
        timer_node = (tiny_timer_node *)pq_min(&timer);
...
void add_timer(http_request_t *rq, size_t timeout, timer_handler_pt handler) {
    int rc;
    tiny_timer_node *timer_node = (tiny_timer_node *)malloc(sizeof(tiny_timer_node));
...
void del_timer(http_request_t *rq) {
    debug("in del_timer");
    time_update();
    tiny_timer_node *timer_node = rq->timer;
